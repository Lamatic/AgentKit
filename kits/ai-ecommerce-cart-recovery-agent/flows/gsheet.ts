/*
 * # GSheet
 * A flow that ingests rows from a Google Sheet, converts them into vector embeddings, and indexes them into the shared knowledge base used by the wider AI E-Commerce Cart Recovery Agent  system.
 *
 * ## Purpose
 ...
 * - Do not use when you only need to query the knowledge base; use the downstream `AI E-Commerce Cart Recovery Agent ` flow after indexing is complete.
 ...
 *
 * ### Downstream Flows
 * - `AI E-Commerce Cart Recovery Agent ` flow — consumes the indexed knowledge indirectly by querying the shared vector database after this flow has populated or refreshed it.
 ...
 *
 *  6. `Index to DB` (`dynamicNode`) writes the generated `vectors` and `metadata` into the selected `vectorDB`. The node uses `chunk_id` as the configured primary key and is configured with `duplicateOperation` set to `overwrite`, so records with the same `chunk_id` are updated instead of duplicated.
 *
 ...
 * | Indexed rows overwrite unexpectedly | `duplicateOperation` is `overwrite` and multiple records generate the same `chunk_id` | Ensure every indexed chunk generates a stable and unique `chunk_id` so unrelated records do not overwrite each other. |
 * | Downstream chatbot returns no relevant answers after a successful run | The downstream `AI E-Commerce Cart Recovery Agent ` flow is querying a different vector store, wrong namespace, or stale index context | Confirm that this flow and the chatbot use the same vector database configuration and retrieval scope. |
 ...
 * -  The indexing node uses `chunk_id` as its sole primary key. Records with the same `chunk_id` overwrite existing records, so every indexed chunk should have a stable and unique identifier..
...
export const nodes = [
  ...
  },
  {
    ...
    "data": {
      ...
      "values": {
        "nodeName": "Variables",
       "mapping": "{\n  \"title\": {\n    \"type\": \"string\",\n    \"value\": \"Data\"\n  },\n  \"source\": {\n    \"type\": \"string\",\n    \"value\": \"{{triggerNode_1.output.spreadSheetLink}}\"\n  }\n}"
      }
    }
  }
];
