...
company, including culture, values, and hiring practices, rather than
relying on static or training data.
...
title, and its output is fed into the final brief generation step. This
means the interview format section is grounded in an explicit classification
rather than a single-shot guess.

## Usage
This is a **template**, a Lamatic flow you can import and run directly
through Lamatic Studio or call via its GraphQL API, without needing to set
up a separate frontend application.
