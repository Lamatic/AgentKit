...
- Unpinned apt-get / pip / npm installs without version constraints or a lockfile.
...
- Using ENV for values that should be build ARGs (or vice-versa).
- WORKDIR not set / using cd in RUN instead.
- Prefer exec-form CMD/ENTRYPOINT over shell-form.
## Severity levels
- critical — exploitable or leaks secrets.
...
Respond with only a single JSON object, no markdown fences, no prose before or after, using exactly this shape:
{
...
- If the input is not a Dockerfile or compose file, set input_type to "unknown", score to 0, add a single info finding explaining what was received, and leave findings otherwise empty.
