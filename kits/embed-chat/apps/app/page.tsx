...
export default function EmbeddedChatKit() {
  ...
  const handleDocumentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      sessionStorage.setItem("chatkit_source_type", "document")
      sessionStorage.setItem("chatkit_title", title.trim())
      sessionStorage.setItem("chatkit_blob_url", uploadedPdfUrl)
  ...
  const handleWebpagesSubmit = async (e: React.FormEvent) => {
    ...
    sessionStorage.setItem("chatkit_source_type", "webpages")
    sessionStorage.setItem("chatkit_urls", JSON.stringify(urls))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-indigo-100 dark:from-gray-900 dark:via-purple-900/20 dark:to-blue-900/20 text-foreground flex flex-col">
        ...
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold tracking-tight select-none">
              <span className="text-black dark:text-white">Embedded Chat</span>
          ...
          <div className="max-w-2xl w-full">
            <div className="text-center mb-12">
              ...
              <p className="text-xl text-muted-foreground">
                Upload a PDF or provide webpage URLs to create your embedded chatbot
              </p>
            </div>
            ...
            <div className="w-full h-[calc(100vh-12rem)] relative">
              <Card className="h-full backdrop-blur-sm bg-white/90 dark:bg-gray-900/90 border-white/20 shadow-xl overflow-hidden relative">
                {sourceType === "document" && pdfUrl ? (
                  <embed src={pdfUrl} type="application/pdf" className="w-full h-full" title="PDF Preview" />
                ) : (
                  urls.length > 0 && (
                ...
                {isProcessing && !showSuccess && (
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-500/30 via-blue-500/30 to-indigo-500/30 backdrop-blur-sm flex items-center justify-center">
                    <div className="text-center">
                      <div className="relative w-24 h-24 mx-auto mb-6">
                        <Sparkles className="w-24 h-24 text-purple-600 animate-pulse absolute inset-0" />
                        <Sparkles className="w-16 h-16 text-blue-600 animate-ping absolute inset-0 m-auto" />
                        <Sparkles className="w-12 h-12 text-indigo-600 animate-bounce absolute inset-0 m-auto" />
                      </div>
                ...
                {showSuccess && (
                  <div className="absolute inset-0 bg-gradient-to-br from-green-500/30 via-emerald-500/30 to-teal-500/30 backdrop-blur-sm flex items-center justify-center animate-in fade-in duration-500">
                    <div className="text-center">
                      ...
                      <h3 className="text-2xl font-bold mb-2 text-green-900 dark:text-green-100">Ready to Chat!</h3>
                      <p className="text-muted-foreground">Redirecting to your chatbot...</p>
                    </div>
                  </div>
                )}
              </Card>
