...
- `reroutedDepartureOffsetHours`: for cancellations only — how many hours EARLIER the replacement flight departed than the original scheduled departure. 0 if it departed at the original time or later; negative values are valid. Use the sentinel -999 if there was no rerouting or the comparison cannot be determined — never -1, which is a real value here.
