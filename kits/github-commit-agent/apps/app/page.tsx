...
import { runCommitAgent } from "@/actions/orchestrate"
import ReactMarkdown from "react-markdown"
import remarkGfm from "remark-gfm"
import { Header } from "@/components/header"

const STAGES = [
  { label: "Parsing message intent" },
  { label: "Fetching repository commits" },
  { label: "Generating markdown summary" },
]

export default function GitCommitAgentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!query.trim()) {
      setError("Please enter a repository or query")
      return
    }
    ...
    try {
      const response = await runCommitAgent(query)
    ...
    <div className="min-h-screen bg-[#FAFAF8] text-neutral-900">
          ...
          <div className={`transition-all duration-700 ease-out ${animateIn ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <div className="text-center mb-16">
              ...
              <p className="text-base text-neutral-500 max-w-md mx-auto leading-relaxed">
                Generate clean, human-readable release notes and commit summaries from any public repository using natural language.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="max-w-xl mx-auto">
              <div className="relative group">
                ...
                <div className="relative flex items-center bg-white rounded-full border border-neutral-200 shadow-sm group-focus-within:shadow-md group-focus-within:border-neutral-300 transition-all duration-300">
                  ...
                  <Input
                    id="query"
                    placeholder="e.g. What changed in Lamatic/AgentKit since v1.0.0?"
                    value={query}
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-10">
              {["Lamatic/AgentKit", "vercel/next.js", "facebook/react", "vuejs/core"].map((suggestion) => (
                <button
                  key={suggestion}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between mb-10">
              <div>
                ...
                <p className="text-sm text-neutral-400 mt-1">What GitHub Commit Agent generated</p>
              </div>
              <button
                onClick={handleCopy}
      ...
      <footer className="border-t border-neutral-100 py-8 mt-20">
        <div className="max-w-6xl mx-auto px-8 flex items-center justify-between">
          ...
          <p className="text-xs text-neutral-300 tracking-wide">GitHub Commit Agent</p>
        </div>
      </footer>
    </div>
  )
