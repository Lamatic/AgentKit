...
The flow lives in [`flows/incident-log-postmortem.ts`](./flows/incident-log-postmortem.ts). Import it into [Lamatic Studio](https://studio.lamatic.ai), set a model on each of the 5 LLM nodes, and **Deploy** the flow. Copy the deployed **Flow ID**.

### 2. Run the app

```bash
cd kits/incident-log-postmortem/apps
