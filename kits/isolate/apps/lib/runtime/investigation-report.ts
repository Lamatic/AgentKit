...
export const investigationReportSchema = z.object({
  outcome: z.enum(["likely_reproduced", "not_reproduced", "inconclusive"]),
...
export function parseInvestigationReport(value: unknown) {
  if (typeof value === "object" && value !== null) {
    ...
    return investigationReportSchema.parse(wrapped.report ?? value);
  }
  ...
  return investigationReportSchema.parse(parsed.report ?? parsed);
}
