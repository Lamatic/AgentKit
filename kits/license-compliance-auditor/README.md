...
1. **Input:** The flow takes `dependency_licenses` and an optional `allow_list` (comma-separated SPDX ids) via GraphQL. `dependency_licenses` is declared as a `string` in the trigger schema, so over the wire it must be a **JSON-encoded string** containing an array of dependency objects — not a nested JSON array. See `samples/test_flow.js`, which reads the sample fixture file as raw text (already a JSON string) and sends it as-is. Each object needs a `name`/`Name`, `version`/`Version`, and `license`/`License` field — both casings are accepted, so `pip-licenses --format=json`'s native capitalized keys work directly.
