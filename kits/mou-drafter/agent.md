...
## Overview
This project solves the problem of drafting first-version Memorandum of Understanding (MoU) and vendor agreements from structured form inputs, incorporating 15 legal risk-management patterns. It implements a **single-flow** AgentKit pipeline that validates inputs, calls a Gemini/Claude model to generate clause JSON using prompt-injection defenses, and then deterministically compiles the clauses into a clean LaTeX document. A Next.js web application invokes the flow via Lamatic's SDK, renders warnings and pattern reports, and compiles the LaTeX source to PDF directly in the browser.
...
- **Expected input shape:** See the Input Schema section in the kit's README. Contains 22 structured form inputs, including party details, fees, event dates, and risk toggles.
...
| PDF preview is blank or compile hangs | A LaTeX syntax error occurred or a loop macro was processed | The compilation worker times out after 30 seconds. Check the LaTeX source tab for syntax issues. |
