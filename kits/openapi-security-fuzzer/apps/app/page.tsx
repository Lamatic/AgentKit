...
import { generatePayloads, executeAndAnalyze } from "@/actions/orchestrate"
...
export default function SecurityFuzzerPage() {
  ...
  const handleRunTests = async (e: React.FormEvent) => {
    ...
    try {
      const response = await executeAndAnalyze(openapiSpec, generatedPayloads, baseUrl, authHeader)
      if (response.success) {
        setAnalysisReport(response.data)
        setActiveTab('results')
      } else {
        setError(response.error || "Failed to execute tests or analyze results.")
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "An unexpected error occurred.")
    } finally {
  ...
  const handleExportPayloads = () => {
    ...
    const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(generatedPayloads, null, 2))
  ...
  const handleExportPostmanCollection = () => {
    ...
    const postmanCollection = {
      ...
      item: generatedPayloads.map(payload => ({
