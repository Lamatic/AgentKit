...
- **Trigger:** API request with `changes` (required), and optional `version` and `date`.
...
- **Input constraints:** `changes` must be a non-empty list of PR titles / commit messages, one per line.
