...
- **`apps/package.json` pins `next dev`/`next build` to `--webpack`, not Turbopack.** `apps/lib/lamatic-client.ts` imports `../../lamatic.config` from outside the `apps/` directory root. Turbopack (Next.js 16's default bundler) cannot resolve any module outside its project root, regardless of import syntax — confirmed by testing two workarounds, both of which failed for the same underlying reason: a local mirror file (`apps/lib/lamatic-config.ts` re-exporting the outer config) and a `tsconfig.json` `paths` alias (`@kit/lamatic.config`). Both still required Turbopack to reach outside `apps/` to resolve the real config, which it cannot do. The `--webpack` flag is the working fix and is intentional, not a placeholder — don't remove it without re-solving this. Other kits in this repo with the same `../../lamatic.config` import pattern (e.g. `deep-search`) would need the same flag if run under Turbopack, though this wasn't directly verified on a sibling kit.
