...
`/api/truth` is a mock source of truth (three orders) with the request/response shape the gate expects: `GET /api/truth?ids=PO1430779` → `{ "order": {…}, "offers": [], "eta": null }`. `truth_url` is treated as untrusted input everywhere: it must be `https://`, carry no credentials, point at a public host on the allow-list (`TRUTH_URL_HOSTS` for the app, `TRUTH_HOSTS` inside `codeNode_211`; both default to `cdn.jsdelivr.net`), redirects are refused, the response is read in bounded chunks and rejected past 200 KB, and a fetch that fails for any reason blocks the draft instead of falling back to the drafter's facts. The deployed flow cannot reach `localhost` anyway, so the scenarios point at `assets/truth/PO1430779.json` in this repo, served over `cdn.jsdelivr.net` as a static stand-in with the same shape. Use jsDelivr rather than `raw.githubusercontent.com`: Lamatic's edge runtime gets a 404 from GitHub raw for every path, while the same URLs answer 200 anywhere else. To use the mock route in local mode, expose it over https and add its host to `TRUTH_URL_HOSTS`. A bearer token for a real source goes in the `TRUTH_URL_TOKEN` env var (app) and a `TRUTH_TOKEN` project secret (Studio → Settings → Secrets; `codeNode_211` references it as `{{secrets.project.TRUTH_TOKEN}}` and sends no token while that secret does not exist), never in the URL.
