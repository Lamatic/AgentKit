...
| `verdict: block` with a `truth_url` finding and `audit.fetchError` set | `truth_url` is not reachable from Lamatic's runtime (localhost, private network), redirects, times out, is not JSON or is over 200 KB | Use a public https URL on the allow-list; `assets/truth/PO1430779.json` shows the shape, served through `cdn.jsdelivr.net` (GitHub raw answers 404 to Lamatic's edge). The gate fails closed rather than trusting the drafter's facts |
