...
- `result`: a `CompactResult` — the token-economical digest of the execution, never the raw result set:
  ...
  - `stats`: `{ columns: ColumnStat[] (name, type, min?, max?, avg?, nulls?, distinct?), dateSpan?: { column, min, max } }` — deterministic aggregate statistics computed over the *full* result, even though only a sample of rows is shown to you.
...
4. Write `reasoning` that cites specific numbers from `result` (a stat value, a row count, a sample row's field, a null count) — never a generic restatement of the hypothesis or a claim not traceable to `result`.
