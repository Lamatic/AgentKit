...
🗓️ Weekly Discipline ReportThe numbers: Net <netPnl> · Win rate <winRate as %> · Payoff <payoffRatio>Watch this: <the single biggest leak, with its exact number>→ Rule for next week: <one concrete, testable rule>Keep doing: <one genuine positive from the data>This week's focus: <the single most important change>
