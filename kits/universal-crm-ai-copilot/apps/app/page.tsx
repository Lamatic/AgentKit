...
export default function Page() {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--bg-main)", color: "var(--text-main)", padding: "2rem" }}>
      {/* Header */}
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: "2rem" }}>
        ...
        {/* Input Form */}
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "1rem", padding: "1.5rem", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.5)" }}>
          ...
          <form onSubmit={handleSubmit}>
            <textarea
              ...
              style={{ width: "100%", background: "var(--bg-input)", border: "1px solid var(--border-input)", borderRadius: "0.75rem", padding: "1rem", color: "#f8fafc", fontSize: "0.95rem", resize: "vertical", outline: "none", marginBottom: "1rem" }}
            />
            {errorMessage && (
            ...
            <button
              type="submit"
              disabled={loading}
              style={{ width: "100%", background: "linear-gradient(135deg, var(--accent-primary) 0%, var(--accent-secondary) 100%)", color: "#ffffff", border: "none", borderRadius: "0.75rem", padding: "0.85rem", fontWeight: "700", fontSize: "1rem", cursor: loading ? "wait" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", transition: "opacity 0.2s" }}
            >
          ...
          <div style={{ marginTop: "1.5rem", paddingTop: "1rem", borderTop: "1px solid var(--border-color)" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
              <button
                ...
                style={{ background: "var(--border-color)", border: "none", borderRadius: "0.5rem", padding: "0.4rem 0.75rem", color: "var(--text-muted)", fontSize: "0.8rem", cursor: "pointer" }}
              >
        ...
        {/* Results Panel */}
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: "1rem", padding: "1.5rem", boxShadow: "0 10px 25px -5px rgba(0,0,0,0.5)", display: "flex", flexDirection: "column" }}>
          {!result ? (
            ...
            <>
              ...
              <div style={{ background: "rgba(16, 185, 129, 0.1)", border: "1px solid rgba(16, 185, 129, 0.3)", borderRadius: "0.75rem", padding: "1rem", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem" }}>
                <div>
                  ...
                  <div style={{ fontSize: "1.75rem", fontWeight: "900", color: "var(--success-color)" }}>{result.leadScore} / 100</div>
                </div>
              ...
              <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1rem", borderBottom: "1px solid var(--border-color)", paddingBottom: "0.5rem" }}>
                {(["salesforce", "sap", "zoho", "dynamics365"] as const).map((tab) => (
                  <button
                    ...
                    style={{
                      background: activeTab === tab ? "var(--accent-primary)" : "var(--border-color)",
              ...
              {activePayload && (
                <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                    ...
                    <button
                      onClick={handleCopy}
                      style={{ background: "transparent", border: "1px solid var(--border-input)", borderRadius: "0.4rem", padding: "0.25rem 0.5rem", color: "var(--text-muted)", fontSize: "0.75rem", cursor: "pointer", display: "flex", alignItems: "center", gap: "0.25rem" }}
                    >
                      {copied ? <CheckCircle2 size={12} color="var(--success-color)" /> : <Copy size={12} />}
                      {copied ? "Copied!" : "Copy JSON"}
                    </button>
                  </div>
                  <pre style={{ flex: 1, background: "var(--bg-main)", border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "1rem", color: "#38bdf8", fontSize: "0.85rem", overflowX: "auto", margin: 0 }}>
              ...
              {result.outreach && (
                <div style={{ marginTop: "1.5rem", paddingTop: "1rem", borderTop: "1px solid var(--border-color)" }}>
                  ...
                  <div style={{ background: "var(--bg-input)", border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "0.75rem", fontSize: "0.8rem", color: "#cbd5e1" }}>
