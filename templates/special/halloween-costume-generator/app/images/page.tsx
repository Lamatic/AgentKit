...
import { ChevronLeft, ChevronRight, Share2, Check, ArrowLeft } from "lucide-react"
...
export default function ImagesPage() {
  ...
  async function callLamatic() {
    try {
      ...
      // Only compress if it's base64 and large
      if (!isImageUrl && imageToSend && imageToSend.length > 4_000_000) {
        console.log("[v0] Input image large, compressing before upload…")
        imageToSend = await compressBase64Jpeg(imageToSend, 1024, 0.75)
      }
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <main className="relative z-10 container mx-auto px-6 py-12 flex flex-col items-center justify-center min-h-[calc(100vh-88px)]">
        <div className="space-y-8 max-w-4xl w-full">
          ...
          {shareUrl && (
            <div className="bg-gray-800/50 backdrop-blur-sm border border-purple-500/30 rounded-lg p-4 max-w-2xl mx-auto">
              <p className="text-white text-sm mb-2">Share this link:</p>
              <div className="flex gap-2">
                <input
                  type="text"
                  value={shareUrl}
                  readOnly
                  className="flex-1 bg-gray-900/50 text-white px-4 py-2 rounded border border-gray-700 text-sm"
                />
                <button
                  onClick={copyToClipboard}
                  className="bg-orange-500/90 hover:bg-orange-600 text-white px-4 py-2 rounded transition-all duration-200 flex items-center gap-2"
                >
                  {copied ? (
                    <>
      ...
      {isLoading && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm">
          <p className="text-white text-2xl font-semibold mb-6">
            {isViewingShared ? "Loading shared image..." : "Generating images..."}
          </p>
          <Spinner />
        </div>
      )}
